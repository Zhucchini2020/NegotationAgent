...
class MyAgent(OneShotAgent):
    ...
    def propose(self, negotiator_id: str, state: MechanismState) -> "Outcome":
        ...
        Returns:
            an outcome to offer.
        """
        decay = self.ufun.find_limits(True) #(1-state.relative_time)**0.5
    ...
    def respond(
        ...
        """
        u = self.ufun.eval(offer)
        if u > self._max_received:
            self._best_so_far = offer
            self._max_received = u
        decay = self.ufun.find_limits(True) #(1-state.relative_time)**0.5
        return ResponseType.ACCEPT_OFFER if u > max(self.ufun.reserved_value, decay) else super().respond(negotiator_id, state, offer)
